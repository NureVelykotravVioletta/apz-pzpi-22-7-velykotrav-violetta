...
export const AddWaterForm = ({ onClose, initialAmount, initialDate }) => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(
      addWaterRecord({ amount: Number(amount), time: formatTimeForInput(date) })
    )
      .unwrap()
      .then((response) => {
  ...
  return (
    <ModalOverlay>
      <form className={css.section} onSubmit={handleSubmit}>
        <p className={css.sectionHeader}>Додати води</p>
        ...
        <div className={css.formEditInfo}>
          <div className={css.amountCorrection}>
            <p className={css.enteredData}>Оберіть значення:</p>
            <p>Кількість води:</p>
            <div className={css.amountCalc}>
              ...
              <p className={css.spanAmount}>{amount} мл</p>
              <button
                type="button"
            ...
            <div className={css.inputWrapper}>
              <p className={css.numberTopic}>
                Введіть кількість спожитої води:
              </p>
              <input
                name="amount"
                type="number"
